// vowel formant synthesis, inspired by SubSyn tutorial examples
// current version can avoid clipping as opposed to the original example
...
public:
    ...
    gam::DSF<> mOsc;
    gam::Buzz<> blImpulse; //band limited impulse
    ...
    void init() override {
        ...
        mBWEnv.curve(0);
        mOsc.harmonics(12);
        blImpulse.saw(); //band limited sawtooth impulse
        // We have the mesh be a sphere
        addDisc(mMesh, 1.0, 30);

        createInternalTriggerParameter("amplitude", 0.6, 0.0, 1.0); //0.3
    ...
    virtual void onProcess(AudioIOData& io) override {
        ...
        while(io()){
            ...
            float s1 = blImpulse()*(1-noiseMix) + mNoise()*noiseMix;
            ...
            //apply resonant band pass filter with formant freqs F1, F2, F3, eg:/a/
            /*
            right now there's not potential difference between changing from diff formants of vowels,
            but it does sound completely different(a "car horn" sound) than the original one which is kinda of "trumpet" sound
            potential idea: maybe the original s1 value is not calculated correctly, maybe shouldn't mix with noise;
            */
            mRes1.set(730, 20);
            mRes2.set(1090, 20);
            mRes3.set(2440, 20);
            float tmp = s1;
            s1 = (mRes1(tmp) + mRes2(tmp) + mRes3(tmp)) / 3;
